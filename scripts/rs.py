...
lit_params = {
    'num_col_checks': ceil(common_params['secparam'] / rs_code_params['rate_bits']), 
    'num_row_checks': 1, # ceil(1 / horizontal_code_params['dist']),
    'log_horizontal_k': 18,
    'log_vertical_k': None,
}
lit_params['log_vertical_k'] = ceil(log2(lit_params['num_col_checks'])) + rs_code_params['log_k'] - lit_params['log_horizontal_k'] + 1

# Final code parameters, should just be Reed Solomon
final_code_params = {
    'log_k': max(lit_params['log_horizontal_k'], lit_params['log_vertical_k']),
    'rate_bits': None,
    'dist': None,
}
final_code_params['rate_bits'] = rs_code_params['log_k'] + rs_code_params['rate_bits'] - final_code_params['log_k']
...
codeswitch_params = {
    'log_k': final_code_params['log_k'],
    'rate_bits': rs_code_params['rate_bits'],
}
...
if __name__ == '__main__':
    ...
    queries_to_w_for_lit_reduction = lit_params['num_col_checks'] * lit_params['num_row_checks']
    queries_to_wprime_for_lit_reduction = lit_params['num_col_checks']
    x1 = proof_size(rs_code_params['log_k'], rs_code_params['rate_bits'], common_params['hashsize'], queries_to_w_for_lit_reduction)
    x2 = proof_size(lit_params['log_horizontal_k'], rs_code_params['rate_bits'], common_params['hashsize'], queries_to_wprime_for_lit_reduction)
    ...
    print((x1 + x2 + 0 + 0 + x5 + x6) / sum_stir2.argument_size())